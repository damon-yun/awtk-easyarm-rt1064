...
SDK_ALIGN(uint8_t s_gui_memory[480*272*2], FRAME_BUFFER_ALIGN);
AT_NONCACHEABLE_SECTION_ALIGN(uint8_t s_vram_buffer[480*272*2], FRAME_BUFFER_ALIGN);
#else
uint32_t s_gui_memory[(GUI_NUMBYTES)];
uint32_t s_vram_buffer[VRAM_SIZE * GUI_BUFFERS];
#endif

uint8_t *online_fb_addr = s_vram_buffer;
uint8_t *offline_fb_addr = s_gui_memory;
